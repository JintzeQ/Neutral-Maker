...
async def capture(symbols, seconds, out):
    ...
    for s in symbols:
        x=s.lower()
        streams += [f"{x}@bookTicker", f"{x}@trade"]
    url="wss://fstream.binance.com/stream?streams=" + "/".join(streams)
    path=Path(out); path.parent.mkdir(parents=True, exist_ok=True)
    counts={s:{"bookTicker":0,"trade":0} for s in symbols}
    t0=time.monotonic(); deadline=t0+seconds
    with gzip.open(path,"wt",encoding="utf-8",compresslevel=3) as f:
        f.write(json.dumps({"type":"META","symbols":symbols,"started_ms":int(time.time()*1000),"source":"fstream.binance.com","streams":["bookTicker","trade"]})+"\n")
        async with websockets.connect(url, ping_interval=20, ping_timeout=20, close_timeout=5, max_queue=20000) as ws:
            while time.monotonic()<deadline:
                ...
                et=data.get("e")
                if sym in counts:
                    if et=="trade": counts[sym]["trade"]+=1
